...
- [`sf_as_dbplyr()`](https://arthurgailes.github.io/overtureR/reference/sf_as_dbplyr.md)
  (and so `sf` and `sfc` spatial filters) sends geometry to DuckDB as
  well-known binary instead of well-known text. Uploading 5,000 polygons
  drops from about 14 seconds to under 0.1 seconds, and coordinates keep
  full precision.

- An `sf` spatial filter is now uploaded once and its union kept in a
  small temporary table, instead of leaving a registered data frame and
  two views behind and re-running `ST_Union_Agg()` on every query.

- Connections created by
  [`stage_conn()`](https://arthurgailes.github.io/overtureR/reference/stage_conn.md)
  (and any connection passed to
  [`open_curtain()`](https://arthurgailes.github.io/overtureR/reference/open_curtain.md))
  enable DuckDB’s Parquet metadata and HTTP metadata caches, so repeated
...
- [`open_curtain()`](https://arthurgailes.github.io/overtureR/reference/open_curtain.md)
  ...
  such as `"2026-08-19.0"`, instead of whichever release is latest when
  the script runs. Set `options(overturer_release = )` to pin a whole
  session. The release is recorded on the result, and the new
  [`print()`](https://rdrr.io/r/base/print.html) method for
  `overture_call` objects shows it. `base_url` now defaults to `NULL`
  and is built from `release`.

- [`open_curtain()`](https://arthurgailes.github.io/overtureR/reference/open_curtain.md)
  gains a `predicate` argument: `"intersects"` (the default), `"within"`
  or `"contains"`, so a polygon filter can keep only the features fully
...
- [`record_overture()`](https://arthurgailes.github.io/overtureR/reference/record_overture.md)
  writes an `_overture.json` manifest into each `type=` directory it
  creates, recording the source release and the bounding box of every
  file.
  [`open_curtain()`](https://arthurgailes.github.io/overtureR/reference/open_curtain.md)
  on such a copy reads only the files that touch `spatial_filter`, and
  labels the result with the release it came from.

- [`record_overture()`](https://arthurgailes.github.io/overtureR/reference/record_overture.md)
  gains `partition_by`, for extra partition columns below `theme` and
  `type`, and `grid`, which partitions the copy into cells of a given
  size in degrees so large local copies also skip files by location.

- [`record_overture()`](https://arthurgailes.github.io/overtureR/reference/record_overture.md)
  can download in one call: pass a type name and a `spatial_filter`
  (plus any other
  [`open_curtain()`](https://arthurgailes.github.io/overtureR/reference/open_curtain.md)
  arguments) instead of an `overture_call`.

- [`record_overture()`](https://arthurgailes.github.io/overtureR/reference/record_overture.md)
  with `overwrite = TRUE` replaces the `theme=/type=` directories it is
  about to write instead of writing new files next to the old ones, so a
  re-recorded copy holds only the new rows. Other files in `output_dir`
  are untouched. Written files get unique names (`data_<uuid>.parquet`),
  so DuckDB’s Parquet metadata cache never serves a stale footer for a
  rewritten copy.

- [`record_overture()`](https://arthurgailes.github.io/overtureR/reference/record_overture.md)
  quotes `output_dir`, so a path containing `'` works.
...
- `sf`, `sfc` and `bbox` spatial filters in a coordinate reference
  system other than EPSG:4326 are transformed before filtering.
  Previously their raw coordinates were compared with Overture’s
  longitude and latitude, which silently returned no rows or the wrong
  rows. A filter with no coordinate reference system is assumed to be
  EPSG:4326, with a warning.

- [`stage_conn()`](https://arthurgailes.github.io/overtureR/reference/stage_conn.md)
  now passes `dbdir`, `read_only`, `bigint` and `config` on to duckdb.
  Before, only `...` reached
  [`DBI::dbConnect()`](https://dbi.r-dbi.org/reference/dbConnect.html),
  so `stage_conn(dbdir = "x.duckdb")` silently opened an in-memory
  database.

- [`stage_conn()`](https://arthurgailes.github.io/overtureR/reference/stage_conn.md)
  registers its shutdown finalizer once rather than on every call, and
  [`strike_stage()`](https://arthurgailes.github.io/overtureR/reference/stage_conn.md)
  no longer opens a new connection just to close it when none is cached.

- [`collect()`](https://dplyr.tidyverse.org/reference/compute.html)
  checks the geometry column’s type before converting it. A column that
  is already well-known binary (for example after
  `mutate(geometry = ST_AsWKB(geometry))`) is used as is, and a
  non-spatial column named `geometry` is left alone, where both used to
  fail with a binder error. Geometry is converted with
  [`sf::st_as_sfc()`](https://r-spatial.github.io/sf/reference/st_as_sfc.html)
  rather than through GDAL, which removes the
  `OGR: Unsupported geometry type` message on places data. Extra
  arguments such as `collect(x, crs = 3857)` now work.

- [`latest_overture_release()`](https://arthurgailes.github.io/overtureR/reference/latest_overture_release.md)
  no longer falls back to a hardcoded release when the catalog is
  unreachable, because Overture removes releases after a few months and
  the hardcoded one would itself fail. It now uses the newest release in
  the local catalog cache with a warning, or fails with an error that
  points to `base_url`.
...
- [`record_overture()`](https://arthurgailes.github.io/overtureR/reference/record_overture.md)
  checks its input before touching the connection.
...
- The test suite now runs offline against a few hundred Overture
  features saved under `tests/testthat/fixtures/`, plus a miniature of
  Overture’s STAC catalog. It covers each SQL builder, every filter
  kind, the catalog cache, and a regression test for each fix above.
  Three tests still read the live release; they run locally and on a
