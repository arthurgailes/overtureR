...
- Pin a release with `open_curtain(release = )`, and list the releases Overture
  still hosts
- Custom partitions and tile-by-tile downloads in `record_overture()`
- Download in one call, without `open_curtain()` first
- A `predicate` argument (`"within"`, `"contains"`) alongside the current
  intersects filter
