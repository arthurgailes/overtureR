...
In the code above, we've used `open_curtain()` to fetch the boundary of Washington, DC from the "division_area" dataset. We then filtered for the specific region we wanted. Next, we created a spatial buffer around DC to define our area of interest for subsequent queries. Finally, we located Ronald Reagan National Airport using the "place" dataset, filtering by name and category.

Now that we've set the stage with our starting point, let's find our destination. In the next code block, we'll locate the Kennedy Center:
...
Let's bring it all together with a visualization. In the next code block, we'll create a map that displays all the elements we've gathered:
